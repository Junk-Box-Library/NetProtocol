================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: NetProtocol プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した NetProtocol には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには NetProtocol アプリケーションを構成している各ファイ
ルの概要説明が含まれます。
...
NetProtocol.h
   ...
   CNetProtocolApp アプリケーション クラスを宣言します。

NetProtocol.cpp
   これは、アプリケーション クラス CNetProtocolApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\NetProtocol.ico
   ...
   アイコンはメイン リソース ファイル NetProtocol.rc に含まれていま
   す。
...
res\Toolbar.bmp
   ...
   プをリソース エディタを使用して編集し、NetProtocol.rc ファイル内
...
NetProtocolDoc.h, NetProtocolDoc.cpp - ドキュメント
   これらのファイルは CNetProtocolDoc クラスを含みます。特別なドキュメント 
   データを追加したり、ファイルを保存したり、
   (via CNetProtocolDoc::Serialize) を読み込んだりするには、これらのファイ
   ルを編集してください。

NetProtocolView.h, NetProtocolView.cpp - ドキュメントのビュー
   これらのファイルは CNetProtocolView クラスを含みます。
   CNetProtocolView オブジェクトは CNetProtocolDoc オブジェクトを表示す
   るのに使用されます。
...
StdAfx.h, StdAfx.cpp
   ...
   NetProtocol.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
